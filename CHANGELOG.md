...
- 新增 `系統診斷` Google Sheet 分頁；瀏覽器端診斷紀錄會在連線恢復後自動批次回寫，並以 DiagnosticID 去重、保留最近 1000 筆
...
- 清除本機設定時保留尚未同步的診斷紀錄，避免排錯證據一併遺失

- 更新前端、Apps Script 與靜態資源快取識別為 `v1.0 RC4`，方便確認正式網站是否已更新
...
- 設定頁新增 Apps Script 連線測試、使用者重新命名、隱私與本機設定清除，以及 App 版本資訊
...
- 新增前後端 RequestID 診斷紀錄，協助判斷 HTTP 400、404 與 Apps Script 實際寫入階段

- 新增 RecordID 冪等防重；寫入成功但回應遺失後再次送出，不再產生重複紀錄
...
- 修正 iOS Safari 因使用者選項按鈕包在 `<label>` 內而無法點選切換使用者；改用正確的按鈕群組結構、加入行動裝置觸控最佳化，並更新靜態資源版本以避免舊快取
