...
- 舒張壓 ≥ 90 時只標紅舒張壓。
- 脈搏 < 50 或 > 100 時只標紅脈搏。
- 燈號狀態與數字紅字分開判斷，不再因整筆燈號狀態將所有數值一起標紅。
- 設定頁需公開顯示以上門檻。

### 原因

逐項標示可讓使用者與醫師立即辨識實際超出門檻的項目，避免誤以為同一筆紀錄內的所有數值都有問題。

### 後續影響

若未來調整脈搏或血壓紅字門檻，首頁、歷史、趨勢、設定頁與 UAT 必須同步修改。
...
- 設定頁的連線、規則與初次教學區域預設收合。
- 尚未設定 Apps Script 網址時，啟動後自動前往設定頁並展開連線區域。
- 未設定網址時不發送首頁與遠端姓名 API 請求。
- 已設定網址時維持原本首頁啟動流程。

### 原因

收合次要內容可縮短設定頁；首次連線引導可避免使用者停留在無法儲存資料的首頁，並減少無效 API 錯誤。
...
長者可能因光線、視力或裝置設定需要固定顯示模式，讓使用者明確選擇比跟隨系統更容易預期。

---

# ADR-015

## 底部功能列背景延伸至 iPhone 安全區
...
- 功能列使用 `bottom: 0` 貼齊視窗。
- 安全區納入功能列高度與底部內距，不作為整條功能列的位移。
- 鍵盤開啟時以完整功能列高度移出畫面。

### 原因

將 `safe-area-inset-bottom` 用於 `bottom` 會讓 iPhone 圓角機型的功能列向上漂浮，並在最下方留下沒有背景的空隙。
