...
新增紀錄採用冪等寫入：前端先建立 UUID，並同時作為 RecordID 與 RequestID。Apps Script 在 LockService 保護範圍內先檢查 RecordID；相同內容的重送回傳成功但不新增資料列，避免寫入成功後 HTTP 回應遺失造成重複紀錄。

API 診斷分為兩層：前端本機循環保存最近 50 次去識別化請求結果，Apps Script 以相同 RequestID 輸出結構化階段紀錄。兩邊皆不得記錄姓名、血壓值或完整部署網址。
